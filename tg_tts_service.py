...
if __name__ == "__main__":
	...
	for k,v in tqdm(voice_name_mapping.items()):
		model.latent_cache[k] = torch.load("./speaker_latents/" + k + ".speaker_latent")
	print("Cached voices.")
	...
	with tts_lock:
		...
		audio_list, sr = model.generate_voice_clone_tg(
			...
			ref_speaker=voice_name_mapping_reversed[list(voice_name_mapping.values())[0]]
		)
		sf.write(trash, audio_list[0], sr, format="wav")
		input_audio, _ = lava_model.load_audio(io.BytesIO(trash.getvalue()), input_sr=24000)
		output_audio = lava_model.enhance(input_audio, denoise=False).cpu().numpy().squeeze()
		del trash
		del input_audio
		del output_audio
